...
The closest we get to our ideal persistence is by using an O/R mapping framework such as [Hibernate](http://sourceforge.net/projects/hibernate/files/hibernate2/), that persists object state to a relational database. The following steps are typically performed when a user interacts with a data entry use case
